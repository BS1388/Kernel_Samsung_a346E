...
> **حل شد (۲۰۲۶-۰۹-۰۸): این راهنما دیگه لازم نیست.**
> علت واقعی «ماژول ناسازگار» نبود؛ ماژول‌های vendor استوک روی کرنل ما درست لود می‌شدن.
> مشکل، ماژول‌های `/system_dlkm` سامسونگ بودن (`bluetooth.ko`, `rfcomm.ko`, `hidp.ko`,
> `hci_uart.ko`, `btsdio.ko`, `btbcm.ko`, `btqca.ko`) که با `CONFIG_KDP_CRED=y` (Knox KDP)
> بیلد شدن و به سیمبل‌های `kdp_usecount_inc` / `kdp_usecount_dec_and_test` /
> `kdp_set_cred_non_rcu` نیاز دارن — چیزی که تو GKI خام وجود نداره:
> ```
...
>
> متن پایین فقط برای تاریخچه/عیب‌یابی نگه داشته شده. توجه: ورک‌فلو دیگه آرتیفکت
> `kernel-modules-*` نمی‌سازه (فقط `Image` آپلود می‌شه).
